...
if question == 'What products increase the chance of buying other products?':
    ...
    st.markdown('## What products increase the chance of buying other products?')
    ...
    col_im.image("Streamlit Data/Images/equations-mba.png",
    ...
    st.markdown('''
                ...
                the confidence (to measure reliability) and popularity. Use the cutoff values below to generate an association rule. 
                ''')

    col1, col2, col3 = st.beta_columns(3)

    lift = col1.number_input(
        label="Choose Lift cutoff value", min_value=1.0, max_value=4.0, value=2.0)
    conf = col2.number_input(label="Choose Confidence cutoff value", min_value=0.0,
                             max_value=1.0, value=0.4)
    support = col3.number_input(label="Choose Support cutoff value", min_value=0.0,
...
if question == 'What customers are we likely to lose soon?':
    ...
    acc_length = col4.number_input("Account length", min_value=0, step=1)
    int_plan = col5.number_input(
        "International Plan", min_value=0, max_value=1, step=1)
    vml_plan = col6.number_input(
        "Voice Mail Plan", min_value=0, max_value=1, step=1)
    num_vm = col7.number_input(
        "Number of Voice Mail Messages", min_value=0, step=1)
    total_day = col8.number_input("Total Day Minutes", min_value=0)
